...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'dark' : ''} bg-white dark:bg-gray-900 font-sans`}>
      ...
      <nav className="fixed w-full z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md shadow-md border-b border-amber-100 dark:border-gray-800 transition-colors">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="hidden md:flex flex-1 max-w-lg mx-8 relative">
            <input type="text" placeholder="Search components..." className="w-full pl-10 pr-4 py-2 rounded-full bg-gray-100 border border-gray-200 dark:bg-gray-800 dark:border-gray-700 focus:ring-2 focus:ring-amber-500 text-gray-800 dark:text-gray-200" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            <i className="fas fa-search absolute left-4 top-3 text-gray-400"></i>
          </div>
          <div className="flex items-center space-x-4">
            <button onClick={toggleTheme} className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 transition-colors">
              <i className={`fas ${isDarkMode ? 'fa-sun text-yellow-400' : 'fa-moon text-gray-600'}`}></i>
            </button>
            <button onClick={() => setIsCartOpen(true)} className="relative p-2 rounded-full bg-gray-100 dark:bg-gray-800 transition-colors">
      ...
      <main className="container mx-auto px-4 py-8 pb-24 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {isLoading ? (
          <div className="col-span-full flex flex-col items-center py-24">
            ...
            return (
              <div key={product.id} className="animate-pop-in bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg border border-gray-200 dark:border-gray-700 group hover:border-amber-500 hover:shadow-2xl transition-all flex flex-col h-full">
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-pop-in">
            <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 bg-gray-50 dark:bg-gray-800 flex gap-3">
                    <button onClick={() => setIsOrderSummaryOpen(false)} className="flex-1 py-3 rounded-xl font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-200 transition">Edit</button>
                    <button onClick={confirmOrder} className="flex-[2] bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2">Confirm Order <i className="fab fa-whatsapp"></i></button>
                </div>
            </div>
        </div>
      )}
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-gray-900 h-full shadow-2xl flex flex-col p-6 animate-pop-in">
            <h2 className="text-xl font-black text-gray-900 dark:text-white mb-6 border-b pb-4">Your Cart ({cartCount})</h2>
            <div className="flex-1 overflow-y-auto space-y-4">
              {cart.length === 0 ? (<p className="text-center py-20 opacity-50">Your cart is empty.</p>) : (
